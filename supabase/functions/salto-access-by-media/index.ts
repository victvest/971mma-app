...
type SaltoAccessRequest = Record<string, unknown>;

function readString(payload: SaltoAccessRequest, keys: string[]): string | null {
...
function mapAccessErrorMessage(error: MbError): string {
  switch (error.code) {
    case 'TOKEN_EXPIRED':
      return 'QR expired. Refresh your pass in the app.';
    case 'TOKEN_INVALID':
      return 'Invalid member QR.';
    case 'BAD_REQUEST':
...
Deno.serve(async (req) => {
  ...
  let body: SaltoAccessRequest = {};

  try {
    body = (await req.json().catch(() => ({}))) as SaltoAccessRequest;
    const rawDeviceId = readString(body, ['DeviceId', 'deviceId']);
    const deviceId = rawDeviceId ?? 'unknown';
    const requestType = readString(body, ['Type', 'type']) ?? 'QR';
    const type = normalizeGateAttemptType(requestType);
    const data = readString(body, ['Data', 'data']) ?? '';
    ...
    if (!data || !rawDeviceId) {
      ...
      await recordGateAccessAttempt(svc, {
        ...
        console.warn('[salto-access-by-media] failed to log bad request', attemptError);
      });
      return jsonResponse(response, {}, req);
    }

    if (type !== 'QR') {
      const response = buildFailureResponse('Unsupported media type for this access point.');
      await recordGateAccessAttempt(svc, {
        deviceId,
        type,
        requestType,
        ...
        console.warn('[salto-access-by-media] failed to log unsupported type', attemptError);
      });
      return jsonResponse(response, {}, req);
    }

    const decision = await evaluateGateAccess({
      svc,
      deviceId,
      token: data,
      rawType: requestType,
    ...
    if (decision.shouldEnqueueMembershipRefresh && decision.memberUserId) {
      await enqueueMembershipRefreshJob(
        ...
        'salto_live_access_refresh',
      ).catch((jobError) => {
        console.warn('[salto-access-by-media] failed to enqueue membership refresh', jobError);
      });
    }
    ...
    await recordGateAccessAttempt(svc, {
      ...
      console.warn('[salto-access-by-media] failed to log access attempt', attemptError);
    });

    return jsonResponse(response, {}, req);
  } catch (error) {
    const type = normalizeGateAttemptType(readString(body, ['Type', 'type']) ?? 'QR');
    ...
    await recordGateAccessAttempt(svc, {
      deviceId,
      type,
      requestType: readString(body, ['Type', 'type']) ?? 'QR',
      granted: false,
      message: response.Message,
      ...
      console.warn('[salto-access-by-media] failed to log exception', attemptError);
    });
