...
import { syncMemberDisciplinesFromMembershipMirror } from './membershipDisciplines.ts';
...
function mapContract(userId: string, item: MbContract, syncedAt: string): MirrorRow | null {
  ...
  // Mindbody AutopayStatus: Active | Inactive | Suspended.
  // Inactive = canceled / not billing → expired. Suspended = paused.
  const suspended = autopay.includes('suspend');
  const inactive = autopay.includes('inactive');
  const status = terminationDate || isPast(endDate) || inactive
    ? 'expired'
    : suspended
      ? 'paused'
      : 'active';
...
export async function readMembershipSummaryFromMirror(
  svc: SupabaseClient,
  userId: string,
...
async function resolveClientId(svc: SupabaseClient, userId: string): Promise<string> {
  ...
  return link.mindbody_client_id;
}

/** Live Mindbody eligibility for gate access (not the cached mirror). */
export async function fetchLiveMembershipEligibility(
  svc: SupabaseClient,
  mindbodyClientId: string,
): Promise<{ eligible: boolean; planName: string | null }> {
  const query = new URLSearchParams({ 'request.clientId': mindbodyClientId });
  ...
  const activeMemberships =
    membershipsResponse.ClientMemberships ?? membershipsResponse.ActiveClientMemberships ?? [];

  for (const item of activeMemberships) {
    const mapped = mapActiveMembership('gate-check', item, syncedAt);
    if (mapped) rows.push(mapped);
  }

  for (const item of contractsResponse.Contracts ?? []) {
    const mapped = mapContract('gate-check', item, syncedAt);
    if (mapped) rows.push(mapped);
  }

  const summary = pickSummary(rows, syncedAt);
  return {
    eligible: summary.status === 'active',
    planName: summary.planName,
  };
}

export async function refreshMembershipMirror(
  svc: SupabaseClient,
  userId: string,
  ...
  try {
    ...
    // Membership mirror remains authoritative even if derived disciplines fail.
  }

  return { summary, rows, disciplinesSynced };
}
