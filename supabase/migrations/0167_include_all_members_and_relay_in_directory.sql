-- 0167_include_all_members_and_relay_in_directory.sql
-- Allow all registered users (including Apple relay and all app roles) in admin_list_app_member_directory
...
begin
  ...
  if v_membership_filter not in ('all', 'active', 'inactive', 'expired') then
  ...
  with filtered as (
    select
      ...
      p.account_status,
      case
        when coalesce(uam.is_active, false) = true then 'active'
    ...
    where (
        v_role is null
        or p.role = v_role
        or (v_role = 'member' and (p.role is null or p.role = 'guest'))
      )
      and (
        v_linked_filter = 'all'
      ...
      and (
        ...
        or (
          ...
          and (
            coalesce(uam.is_active, false) = true
            or lower(coalesce(p.membership_status, '')) in ('active', 'current')
          )
        )
        or (
          v_membership_filter in ('inactive', 'expired')
          and (
            coalesce(uam.is_active, false) = false
            and (
              lower(coalesce(p.membership_status, '')) in ('expired', 'inactive', 'suspended', 'cancelled', 'terminated')
              or (
                ml.mindbody_client_id is not null
                and lower(coalesce(p.membership_status, '')) not in ('active', 'current')
                and coalesce(p.membership_status, '') <> ''
              )
            )
          )
        )
