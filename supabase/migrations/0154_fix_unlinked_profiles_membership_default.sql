-- Fix unlinked member profiles showing membership_status = 'active'
-- A member only has an active membership if they are linked to Mindbody or have Unlimited VIP.

-- 1. Remove the legacy 'active' default on profiles.membership_status
alter table public.profiles alter column membership_status drop default;
alter table public.profiles alter column membership_status set default null;

-- 2. Clean up unlinked profiles that have no Mindbody link and no VIP pass
...
where not exists (
  ...
  and not exists (
    select 1 from public.unlimited_access_members uam
    where uam.user_id = p.id
      and uam.is_active = true
  )
  and p.membership_source is null;

-- 3. Update admin_list_app_member_directory to compute effective membership status
