-- Harden redeem_reward inventory decrement under concurrent load.
-- Row is already locked FOR UPDATE; still require inventory > 0 on the UPDATE
-- so a concurrent path can never write a negative stock count.
