-- Home dashboard candidates: include today's Gym Usage + other live/upcoming classes.
-- Exclude other-day Gym Usage so the member hero never leads with tomorrow's open floor.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by
        case
          when (c.starts_at at time zone 'Asia/Dubai')::date = v_today
            and (
              lower(c.title) like '%gym usage%'
              or lower(coalesce(c.discipline, '')) like '%gym usage%'
            )
          then 0
          else 1
        end,
  ...
  from (
    ...
    where c.mindbody_class_id is not null
      ...
      -- Drop other-day Gym Usage from the home candidate pool.
      and not (
        (c.starts_at at time zone 'Asia/Dubai')::date > v_today
        and (
          lower(c.title) like '%gym usage%'
          or lower(coalesce(c.discipline, '')) like '%gym usage%'
        )
  ...
  from (
    ...
    where mindbody_staff_id is not null
      ...
      and coalesce(visible_in_app, true) = true
