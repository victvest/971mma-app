-- Member home carousel: today's Gym Usage first, then next live/upcoming classes (up to 3).
-- No member_disciplines filter. Schedule tab and coach mode are unaffected.
...
begin
  ...
  -- Wider candidate pool; client pins Gym Usage and slices to 3.
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'id', c.id,
