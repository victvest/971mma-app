-- Home hero carousel: show academy-wide upcoming classes (no member_disciplines filter).
-- Gym Usage pinning still happens client-side via resolveHomeHeroClasses.
...
begin
  ...
  from (
    ...
    -- Prefer Gym Usage so the home hero matches client-side resolveHomeHeroClasses.
    order by
      case
        when lower(c.title) like '%gym usage%'
