-- Phase 3: Add columns to check_ins for Mindbody visit status alignment and recomputation logic correctness.
alter table public.check_ins
  add column if not exists signed_in boolean not null default true,
  add column if not exists missed boolean not null default false,
  add column if not exists late_cancelled boolean not null default false;

-- Re-define count_training_days to filter out missed/late-cancelled visits
...
-- Re-define recompute_streak to filter out missed/late-cancelled visits
...
-- Re-define evaluate_milestones to filter out missed/late-cancelled visits
...
-- Re-define recompute_discipline_score to filter out missed/late-cancelled visits
...
-- Re-define list_promotion_candidates to filter out missed/late-cancelled visits
...
-- Re-define award_check_in_points to not award points for missed or late_cancelled visits
...
begin
  ...
  -- Check if check-in is missed or late_cancelled
  if exists (
    select 1
    from public.check_ins
    where id = p_checkin
...
-- Create update trigger to recompute points and streaks if checkin status changes
...
declare
  ...
  v_tier text;
begin
  -- If missed or late_cancelled changed to true from false
  if (new.missed = true or new.late_cancelled = true) and (old.missed = false and old.late_cancelled = false) then
    ...
    if v_points_awarded then
      ...
      v_new_balance := greatest(0, v_balance - 10);
      v_new_lifetime := greatest(0, v_lifetime - 10);
      v_tier := case
        when v_new_lifetime >= 4000 then 'gold'
        when v_new_lifetime >= 1500 then 'silver'
        else 'bronze'
      ...
      -- Delete check_in points from ledger
      delete from public.points_ledger
      where reason = 'check_in'
        and ref_id = new.id;
    end if;

  -- If missed/late_cancelled changed from true to false (i.e. made valid checkin)
  elsif (new.missed = false and new.late_cancelled = false) and (old.missed = true or old.late_cancelled = true) then
    perform public.award_check_in_points(new.user_id, new.id);
  end if;

  -- Recompute streak, milestones, discipline score, and belt progress
  begin
...
after update of missed, late_cancelled on public.check_ins
for each row
