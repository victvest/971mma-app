...
function CoachStatsRow({ coach, classCount }: { coach: CoachItem; classCount: number }) {
  ...
  const years =
    coach.yearsCoaching ?? coach.yearsExperience ?? coach.yearsMartialArts ?? null;
      ...
      {items.map((item) => (
        <View key={item} style={styles.listRow}>
          {icon === 'medal' ? (
            ...
            <Shield size={16} color="#007A33" strokeWidth={2.2} />
          ) : (
            <Ionicons name="sparkles-outline" size={16} color="#007A33" />
          )}
...
export default function CoachDetailScreen() {
  ...
  const coachFirstName = useMemo(
    () => (coach?.name ? firstName(coach.name) : 'Coach'),
    [coach?.name],
  );
  ...
  const shareCoach = useCallback(async () => {
    if (!coach) return;
    triggerMediumImpact();
    try {
      const specialty =
        disciplineLabels.length > 0
          ...
          : coach.specialty?.trim() || 'MMA';
      const lines = [coach.name];
      if (coach.rank?.trim()) lines.push(coach.rank.trim());
      lines.push(specialty, '971 MMA — Dubai');
      await Share.share({ message: lines.join('\n') });
    } catch {
      // User dismissed the share sheet.
    }
  }, [coach, disciplineLabels]);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.floatingCardWrap}>
          <View style={styles.floatingCard}>
            <View style={styles.floatingCardContent}>
              ...
              {coach.nickname?.trim() ? (
                <Text style={styles.nicknameText}>“{coach.nickname.trim()}”</Text>
              ) : null}
        ...
        <View style={styles.infoContainer}>
          ...
          {coach.statusAchievements.length > 0 ? (
            <InfoSection title="Status & achievements">
              <BulletList items={coach.statusAchievements} icon="medal" />
            </InfoSection>
          ) : coach.titles.length > 0 ? (
            <InfoSection title="Titles & achievements">
              <BulletList items={coach.titles} icon="medal" />
            </InfoSection>
          ) : null}

          {coach.experienceHighlights.length > 0 ? (
            <InfoSection title="Experience">
              <BulletList items={coach.experienceHighlights} icon="experience" />
            </InfoSection>
          ) : null}

          {coach.coachingStyle.length > 0 ? (
            <InfoSection title="Coaching style">
              <BulletList items={coach.coachingStyle} icon="style" />
            </InfoSection>
